...
export const searchAuditLogs = async (req: Request, res: Response) => {
  try {
    ...
    const cacheKey = crypto
      .createHash("md5")
      .update(JSON.stringify({ tags, naturalLanguageQuery }))
      .digest("hex");
    ...
    if (naturalLanguageQuery && isAiEnabled) {
      ...
      try {
        ...
        const promptText = `You are an expert AI assistant that converts a natural language query into a MongoDB query filter. You are an expert AI assistant that converts a natural language query into a MongoDB query filter object.

**Current Date:** ${currentDate}. Use this as a reference for any relative date queries (e.g., "yesterday", "last month").
...
7.  Return ONLY the raw JSON for the filter object. Do not include any extra text, explanations, or markdown.`;
        ...
        const aiResponse = await llm.invoke(promptText);
        const responseText = aiResponse.content.toString();

        // Clean up the response
        const cleanedFromThink = responseText
          .replace(/<think>[\s\S]*?<\/think>/, "")
          .trim();
        const cleanedFromIsoDate = cleanedFromThink.replace(
          /ISODate\((".*?")\)/g,
          "$1"
        );
        const cleanedFromMarkdown = cleanedFromIsoDate.replace(/^```json\s*/, '').replace(/\s*```$/, '');

        // Parse JSON
        const aiFilter = JSON.parse(cleanedFromMarkdown);

        console.log(
          "AI-Generated Filter Part:",
          JSON.stringify(aiFilter, null, 2)
        );
        conditions.push(aiFilter);

        // LangChain chaining approach (commented for direct GenAI)
